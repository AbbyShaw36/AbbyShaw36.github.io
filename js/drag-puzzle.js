window.onload = function() {
	var oStage = getByClass($('dragPuzzle'),'stage')[0];
...
var DragPuzzle = {
			...
			for (var i = 0; i < 16; i++) {
				...
				oLi.onmousedown = function() {
					...
					document.onmousemove = function(ev) {
						...
						var left = oEvent.clientX - that.oBox.offsetLeft - This.offsetWidth/2 + document.body.scrollLeft;
						var top = oEvent.clientY - that.oBox.offsetTop - This.offsetHeight/2 + document.body.scrollTop;
	...
	arrange : function() {
		...
		while(imgArray.length) {
			var oImg = imgArray.splice(Math.floor(Math.random() * imgArray.length),1);
			var oPos = posArray.splice(Math.floor(Math.random() * posArray.length),1);
	...
	isWin : function() {
		for (var i = 0; i < this.imgArray.length; i++) {
			if (parseFloat(this.imgArray[i].style.left) !== this.posArray[i][2] || parseFloat(this.imgArray[i].style.top) !== this.posArray[i][1]) {
				return false;
			}
		};
		return true;
